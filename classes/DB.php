...
class DB
{
        ...
        try {
            $this->pdo = new PDO(
                sprintf('%s:host=%s;dbname=%s',
                    $config->getMandatoryKey('driver'),
                    $config->getMandatoryKey('host'),
                    $config->getMandatoryKey('name')
                ),
                $config->getMandatoryKey('user'),
                $config->getMandatoryKey('password'),
                array(
    ...
    public function execute($query, array $params = []) {
        ...
        if(!$command->execute($normParams)) {
            Logger::error($command->errorInfo());
    ...
    public function remove($tableName, $fieldName = null, $value = null, $removeAll = false) {
        if(is_null($fieldName)
                && is_null($value)
                && $removeAll) {
            $this->execute(sprintf('DELETE FROM `%s`', $tableName));
